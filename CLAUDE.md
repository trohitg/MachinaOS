...
| **[Native LLM SDK](./docs-internal/native_llm_sdk.md)** | Native SDK layer in `services/llm/`: protocol-based providers, config-driven base URLs, and 14 providers (12 cloud plus Ollama / LM Studio; xAI is agent-selectable without a standalone chat-model node). Two of the cloud entries are **gateways** (OpenRouter, LLMTR) — one key fronting many vendors with `vendor/model` ids, which is what puts them in the open-world tuple in `services/llm/config.py` and in `_ROUTER_PROVIDERS` in `services/model_registry.py`. A JSON-driven `supports_model_listing: false` flag covers endpoints that speak the OpenAI chat wire format but ship no `/v1/models` route (Sarvam) — `OpenAIProvider.fetch_models` then serves the curated list and probes the key with a one-token completion. LLMTR is the mirror-image case: its `/v1/models` route exists but is **public**, so listing it proves nothing about the key and `LLMTRCredential._probe` overrides the inherited fetch-models probe with a one-token completion for the opposite reason. Gateways may declare `routes_vendor_prefixed_models`, which splits `_model_policy` in two: **wire shape** (`max_completion_tokens` vs `max_tokens`, whether `temperature` is sendable) resolves against the routed vendor's block, while **thinking** stays the gateway's — a gateway normalizes reasoning its own way and never forwards vendor-native `extra_body`. `use_responses` is always false for a gateway. Every provider is native; `tests/llm/test_langchain_removed.py` locks the retired-dependency end state. |
